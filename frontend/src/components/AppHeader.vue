...
<template>
  <header class="h-16 border-b border-border bg-background/80 backdrop-blur-md flex items-center justify-between px-8 sticky top-0 z-10 shrink-0">
    <div>
      <h1 class="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-accent">
        CT Agent Hub
      </h1>
      <p class="text-xs text-muted-foreground font-medium">{{ dict[uiLang].title }}</p>
    </div>
    <div class="flex items-center gap-3">
      <button @click="toggleLang" class="shadcn-btn shadcn-btn-outline w-10 h-10 p-0 rounded-full font-bold">
        {{ uiLang === 'vi' ? 'VN' : 'EN' }}
      </button>
      <button @click="toggleDark" class="shadcn-btn shadcn-btn-outline w-10 h-10 p-0 rounded-full">
