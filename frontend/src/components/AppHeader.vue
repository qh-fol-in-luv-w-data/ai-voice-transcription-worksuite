...
<div class="flex items-center gap-1 mr-4">
    <button @click="toggleDark" class="text-on-surface-variant hover:text-on-surface hover:bg-surface-variant/30 p-2 rounded-full transition-colors group" title="Toggle Theme">
    ...
    <button @click="toggleLang" class="text-on-surface-variant hover:text-on-surface hover:bg-surface-variant/30 p-2 rounded-full transition-colors group" title="Translate">
    <span class="material-symbols-outlined text-[20px] group-hover:scale-110 transition-transform font-light">translate</span>
    </button>
</div>

<button class="relative text-on-surface-variant hover:text-on-surface p-2 rounded-full transition-colors group">
<span class="material-symbols-outlined text-[24px] group-hover:scale-110 transition-transform font-light">notifications</span>
<div class="absolute top-2 right-2 w-2 h-2 bg-error rounded-full border border-background"></div>
</button>
...
<span class="font-body-md text-[14px] text-on-surface hidden sm:block font-medium group-hover:text-primary transition-colors">{{ currentFullName || currentUser || 'User' }}</span>
<div class="w-10 h-10 rounded-full bg-surface-container overflow-hidden border border-outline-variant/50 group-hover:border-primary/50 transition-colors flex items-center justify-center">
