...
<a @click.prevent="isHistoryModalOpen = true" :class="['flex items-center gap-4 px-4 py-3.5 rounded-xl font-medium transition-all group cursor-pointer', isHistoryModalOpen ? 'text-white font-bold bg-white/10 shadow-sm border border-white/5' : 'text-on-surface-variant hover:text-white hover:bg-white/5']">
...
<!-- History Modal -->
<div v-if="isHistoryModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
  <div class="bg-surface-container border border-outline-variant/30 rounded-3xl p-6 w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-xl font-bold text-white tracking-wide">Lịch sử Cuộc họp</h2>
      <button @click="isHistoryModalOpen = false" class="w-8 h-8 rounded-full hover:bg-white/10 flex items-center justify-center text-on-surface-variant hover:text-error transition-colors">
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-2">
      ...
      <div v-for="meeting in meetingHistory" :key="meeting.name" class="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/5 transition-all group">
        ...
        <div class="flex-1 min-w-0">
          <template v-if="editingMeeting === meeting.name">
            <input v-model="editTitleInput" @keyup.enter="saveEdit(meeting)" @keyup.esc="cancelEdit" class="w-full bg-surface-container-highest border border-primary/50 rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none" autoFocus />
          </template>
          <template v-else>
            <div @click="selectMeetingFromModal(meeting)" class="font-medium text-[14.5px] text-white truncate cursor-pointer hover:text-primary transition-colors">
              {{ meeting.title }}
            </div>
          </template>
        </div>
        ...
        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <template v-else>
            <button @click="startEdit(meeting)" class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 text-on-surface-variant hover:text-white transition-colors" title="Đổi tên">
