...
# ─────────────────────────────────────────────
# NODE 2: Trích xuất Task & Noti bằng GPT-4o
# ─────────────────────────────────────────────
...
Phân loại:
- Task: việc cụ thể được giao, có người thực hiện rõ ràng
- Noti: thông tin/thông báo, không phải việc phải làm
...
def node_create_tasks(state: AgentState) -> dict:
    ...
    items        = [i for i in data.get("items", []) if i["loai"] == "Task"]
...
def node_report(state: AgentState) -> dict:
    ...
    notis   = [i for i in data.get("items", []) if i.get("loai") == "Noti"]
    ...
    if notis:
        print(f"\n📢 {len(notis)} NOTI:")
        for n in notis:
            desig = f" [{n.get('chuc_danh')}]" if n.get("chuc_danh") else ""
            print(f"   [{n['nguoi_thuc_hien']}{desig}] {n['noi_dung'][:70]}")
