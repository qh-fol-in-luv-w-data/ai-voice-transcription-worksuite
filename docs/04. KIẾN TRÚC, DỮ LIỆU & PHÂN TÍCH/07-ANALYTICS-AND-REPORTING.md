...
Tài liệu hóa analytics and reporting cho dự án dựa trên source code hiện tại.
...
| KPI/Report | Definition | Data source | Formula | Refresh | Access |
|---|---|---|---|---|---|
| Meeting processed | Count of `Voice Meeting` completed/analyzed | `Voice Meeting.status` | count by period | TODO | System Manager/User owner |
| AI token usage | Prompt/completion/total tokens | `VOICE Session`, `VOICE AI Call Log` | sum tokens | Real-time/log | System Manager |
| ElevenLabs chars | chars used/remaining | `VOICE AI Call Log`, balance API | sum/latest | Per call | System Manager |
| Task extraction volume | number of items in `tasks_json` | `Voice Meeting.tasks_json` | JSON item count | Per extract | Owner/Admin |
| Error rate | failed actions/chunks | `VOICE Action Log`, `Voice Meeting Chunk` | failed/total | TODO | Operations |
