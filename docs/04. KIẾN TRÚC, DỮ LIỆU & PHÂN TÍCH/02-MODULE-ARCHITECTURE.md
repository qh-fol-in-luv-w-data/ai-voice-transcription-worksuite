...
Tài liệu hóa module architecture cho dự án dựa trên source code hiện tại.
...
| Module | Responsibility | Public interface | Dependencies | Data entities | Security boundary |
|---|---|---|---|---|---|
| API | Whitelisted endpoints and orchestration | `/api/method/voice_app.api.*` | Frappe, AI clients, File | Voice Meeting, VOICE Logs | Frappe auth/owner checks |
| STT | Audio-to-text | `call_gemini_stt`, `call_elevenlabs_stt` | External APIs, GCS optional | raw segments | API keys |
| Speaker | Enrollment/identification | `SpeakerDB`, embedding extraction | Pyannote/remote API | Voice Speaker | Admin/User |
| Task | LLM extraction and ERP sync | `extract_tasks_only`, `create_tasks_to_erp` | OpenAI, Worksuite | Voice Meeting, external Task | Token/session |
| Frontend | User workflow | Vue components/api.js | Frappe API | Browser state | Session/CSRF |
