...
Tài liệu hóa api test cases cho dự án dựa trên source code hiện tại.
...
| Endpoint | Auth | Valid request | Expected | Invalid cases | Authorization | Performance |
|---|---|---|---|---|---|---|
| `transcribe_audio` | Session | audio FormData | processing | no file, invalid mode | Guest denied | long audio timeout |
| `check_meeting_status` | Session | meeting_name | processing/success/error | missing/invalid name | owner review TODO | frequent polling |
| `extract_tasks` | Session | meeting_name/results | processing | missing meeting_name | owner check in async update | long transcript |
| `sync_tasks_to_erp` | Session | tasks[] | success/report | empty tasks | Guest denied | ERP latency |
| `proctoring_webhook` | Optional secret | valid JSON | success | bad JSON/type/secret | allow_guest; secret recommended | burst alerts |
