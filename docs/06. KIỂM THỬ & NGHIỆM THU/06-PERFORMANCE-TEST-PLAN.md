...
Tài liệu hóa performance test plan cho dự án dựa trên source code hiện tại.
...
| Workload | Metric | Target | Data source | Notes |
|---|---|---|---|---|
| 1 short audio | API response enqueue time | TODO | HTTP timing | Should return quickly |
| Long meeting audio | Completion duration | TODO | `Voice Meeting`, logs | Depends STT provider |
| Concurrent uploads | Queue backlog | TODO | Frappe worker/Redis | Need staging |
| Task extraction | LLM latency/tokens | TODO | `VOICE AI Call Log` | Model/provider-dependent |
| Speaker identification | embedding duration | TODO | logs | Local vs remote |
