...
Tài liệu hóa test strategy cho dự án dựa trên source code hiện tại.
...
|---|---|---|
| Validate transcription | Upload, chunk, STT, result persistence | `transcribe_audio`, `Voice Meeting` |
| Validate speaker flow | enroll, identify, map, undo | `Voice Speaker`, speaker APIs |
| Validate task extraction | DOCX export, OpenAI JSON, XLSX | `extract_tasks`, `docx_utils.py` |
| Validate security | auth, CSRF, owner access, secrets | auth/security docs |

Test levels: unit for helpers, integration for Frappe APIs, E2E for SPA workflows, UAT for business acceptance. Formal framework: TODO.
