...
Tài liệu hóa uat scenarios cho dự án dựa trên source code hiện tại.
...
| UAT ID | Process | Actor | Scenario | Steps | Expected | Status |
|---|---|---|---|---|---|---|
| UAT-001 | Meeting transcription | User | Upload real meeting | Upload, wait, review | Transcript acceptable | Pending |
| UAT-002 | Speaker mapping | User | Unknown speaker appears | Map/enroll | Future labels improved | Pending |
| UAT-003 | Task extraction | User | Meeting has action items | Extract, review modal | Tasks match business expectation | Pending |
| UAT-004 | ERP sync | User | Approved task list | Sync | Tasks exist in Worksuite | Pending |
| UAT-005 | Voice-to-task | User | Create task by voice | Record sentence | Task fields populated | Pending |
