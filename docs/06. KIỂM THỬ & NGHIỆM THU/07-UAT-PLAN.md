...
Tài liệu hóa uat plan cho dự án dựa trên source code hiện tại.
...
| Item | Plan |
|---|---|
| Scope | Upload meeting, review speakers, export minutes, extract/sync tasks, voice-to-task. |
| Team | Business owner, key users, QA, PM. |
| Environment | TODO: UAT URL/config. |
| Test data | Non-sensitive audio samples with known speakers and expected task list. |
| Entry | Build deployed, APIs configured, users provisioned. |
| Exit | Critical defects closed, sign-off completed. |
