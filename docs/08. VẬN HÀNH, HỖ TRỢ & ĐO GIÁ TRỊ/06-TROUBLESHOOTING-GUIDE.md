...
Tài liệu hóa troubleshooting guide cho dự án dựa trên source code hiện tại.
...
| Problem | Symptoms | Possible causes | Diagnostic | Fix |
|---|---|---|---|---|
| Transcription stuck | status processing long | queue stopped/provider latency | Frappe logs, worker queue | restart worker/retry/resume |
| Chunk error | Partial Error | STT failure/audio split issue | `Voice Meeting Chunk.error_message` | resume or re-upload |
| Speaker wrong | wrong names | poor sample/threshold | compare Voice Speaker, scores logs | manual map/enroll |
| Export missing | no DOCX/XLSX | extract failed | `check_extract_status`, logs | rerun extract |
| ERP sync failed | error report | token/url/data mismatch | Worksuite API logs | fix config/retry |
