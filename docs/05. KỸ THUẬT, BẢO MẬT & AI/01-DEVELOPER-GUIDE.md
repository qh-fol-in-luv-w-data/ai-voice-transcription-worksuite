...
Tài liệu hóa developer guide cho dự án dựa trên source code hiện tại.
...
- Python >=3.10, Frappe Bench v15+, Node.js 18+, npm/yarn.
- ffmpeg/pydub dependencies for audio conversion.
- API keys/config in `Voice App Settings`, `site_config.json`, bench `.env` or environment.
...
- Python formal tests: NOT FOUND. Existing scripts: `voice_app/test_*.py`, `check_*.py`, debug helpers.
- Coding convention: Ruff configured in `pyproject.toml`.
