...
Tài liệu hóa ci cd pipeline cho dự án dựa trên source code hiện tại.
...
NOT FOUND: Không tìm thấy CI/CD workflow trong workspace.

Proposed pipeline cần xác nhận:

```mermaid
flowchart LR
  A[Push/PR] --> B[Lint Python + JS]
  B --> C[Unit/API tests]
  C --> D[Frontend build]
  D --> E[Security scan]
