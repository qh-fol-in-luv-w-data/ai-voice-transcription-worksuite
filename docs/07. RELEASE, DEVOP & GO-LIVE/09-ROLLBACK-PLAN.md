...
Tài liệu hóa rollback plan cho dự án dựa trên source code hiện tại.
...
| Area | Procedure |
|---|---|
| Conditions | S1/S2 issue, data corruption, security exposure, migration failure. |
| Authority | TODO: DevOps + Tech Lead + Business Owner. |
| Application | Restore previous app revision/artifact, `bench build`, restart. |
| Database | Restore pre-deploy backup if migration/data corruption. |
| Configuration | Revert changed Settings/site_config. |
| Validation | Login, upload small audio, history, export, sync smoke tests. |
