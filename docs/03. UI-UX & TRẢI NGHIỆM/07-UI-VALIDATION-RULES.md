...
Tài liệu hóa ui validation rules cho dự án dựa trên source code hiện tại.
...
| Screen | Field | Required | Frontend validation | Backend validation | Source |
|---|---|---|---|---|---|
| VoiceTranscribe | audio file | Yes | Upload control required by flow | Throws `Thiếu file âm thanh` | `VoiceTranscribe.vue`, `api.py` |
| VoiceTranscribe | filter_speakers | No | JSON list when selected | parsed JSON | `frontend/src/api.js`, `api.py` |
| TaskModal | tasks | For sync yes | warns when empty | returns error if empty | `App.vue`, `sync_tasks_to_erp()` |
| Sidebar | newTitle | Yes | trim empty guard | `_can_access_meeting`, title required | `AppSidebar.vue`, `meeting_api.py` |
| VoiceEnroll | audio sample | Yes | recording/upload flow | file required | `VoiceEnroll.vue`, `enroll_voice()` |
