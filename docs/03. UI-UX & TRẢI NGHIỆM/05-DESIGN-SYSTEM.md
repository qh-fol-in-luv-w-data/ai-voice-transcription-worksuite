...
Tài liệu hóa design system cho dự án dựa trên source code hiện tại.
...
| Element | Observed implementation | Source |
|---|---|---|
| Typography | Inter imported in `App.vue`; Material Symbols used in sidebar. | `frontend/src/App.vue`, `AppSidebar.vue` |
| Components | Element Plus button/table/modal/select/date UI. | `frontend/package.json`, Vue files |
| Color | CSS variables/classes like `primary`, `surface`, dark overrides. | `frontend/src/style.css`, `App.vue` |
| Navigation | Fixed left sidebar desktop, `activeTab` state. | `AppSidebar.vue`, `useVoiceApp.js` |
| Modal | Teleport overlay for history; `TaskModal` global. | `AppSidebar.vue`, `TaskModal.vue` |
| Responsive | Sidebar hidden on mobile, main content scrollable. | `App.vue`, `AppSidebar.vue` |

TODO: Cần review visual tokens trong `frontend/src/style.css` nếu cần design-system chính thức.
