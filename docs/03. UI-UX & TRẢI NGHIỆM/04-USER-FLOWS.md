...
Tài liệu hóa user flows cho dự án dựa trên source code hiện tại.
...
```mermaid
flowchart TD
  A[Open SPA] --> B[get_context]
  B -->|success| C[Authorized app]
  B -->|AuthenticationError| D[Access denied]
```
...
```mermaid
flowchart TD
  A[Upload audio] --> B[Wait realtime/poll progress]
  B --> C[Review transcript]
  C --> D[Map/edit speakers]
  D --> E[Extract tasks]
  E --> F[Review TaskModal]
  F --> G[Export/sync ERP]
```

Registration/password recovery/approval/import/export administration: NOT FOUND custom implementation; handled by Frappe core or not implemented.
