...
Tài liệu hóa ui ux overview cho dự án dựa trên source code hiện tại.
...
- Giao diện vận hành tập trung, ưu tiên hoàn tất workflow: phân tích hội thoại, tạo task, đăng ký giọng, lịch sử.
- SPA dùng sidebar cố định desktop, dark mode, Element Plus components.

### UI technology

| Layer | Source |
|---|---|
| SPA shell | `frontend/src/App.vue` |
| Sidebar navigation | `frontend/src/components/AppSidebar.vue` |
| UI components | Element Plus, Material Symbols, CSS/Tailwind classes |
| Session init | `frontend/src/utils/session.js`, `voice_app/api.py` — `get_context()` |
