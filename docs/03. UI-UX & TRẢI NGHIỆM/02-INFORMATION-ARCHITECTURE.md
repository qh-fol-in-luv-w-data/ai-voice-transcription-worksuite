...
Tài liệu hóa information architecture cho dự án dựa trên source code hiện tại.
...
```mermaid
flowchart TD
  App[2AS Worksuite SPA] --> T[Phân tích Hội thoại]
  App --> V[Tự tạo Task qua Voice]
  App --> E[Đăng ký Giọng nói]
  App --> H[Lịch sử Cuộc họp]
  App --> VH[Lịch sử Giao việc]
  T --> TM[Task Modal]
  H --> TM
```

| Node | Screen/component | Source |
|---|---|---|
| Phân tích Hội thoại | `VoiceTranscribe.vue` | `frontend/src/views/VoiceTranscribe.vue` |
| Tự tạo Task qua Voice | `VoiceTask.vue` | `frontend/src/views/VoiceTask.vue` |
| Đăng ký Giọng nói | `VoiceEnroll.vue` | `frontend/src/views/VoiceEnroll.vue` |
| Lịch sử Cuộc họp | `MeetingHistory.vue`, sidebar modal | `frontend/src/views/MeetingHistory.vue` |
| Task review | `TaskModal.vue` | `frontend/src/components/TaskModal.vue` |
